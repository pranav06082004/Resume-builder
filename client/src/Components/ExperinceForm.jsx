import { Briefcase, Plus, Trash, Trash2, Sparkles, Loader2 } from 'lucide-react'
...
const ExperinceForm = ({data, onChange}) => {
    ...
    try{
        ...
        updateExperience(index, "description", res.data.enhancedContent || res.data.data)
    }
    catch(error){
        console.log(error)
            ...
            <div className='text-center py-8 text-gray-500'>
                ...
                <div className='space-y-4'>
                    {data.map((experience, index) => (
                        <div key={index} className="p-4 border-gray-300 rounded-lg space-y-3">
                            ...
                            <div className='grid md:grid-cols-2 gap-3'>
                                ...
                                <input value={experience.startDate || ""} onChange={(e) => updateExperience(index, "startDate", e.target.value)} type="month" className="px-3 py-2 text-sm rounded-lg"/>

                                <input value={experience.endDate || ""} onChange={(e) => updateExperience(index, "endDate", e.target.value)} type="month" disabled={experience.is_current} placeholder="End Date" className="px-3 py-2 text-sm rounded-lg disabled:bg-gray-100" />
                            ...
                            <div className="space-y-2">
                                <div className='flex items-center justify-between'>
                                    <label className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>Job Description</label>
                                ...
                                <textarea value={experience.description || ""} onChange={(e) => updateExperience(index, "description", e.target.value)} className="w-full text-sm px-3 py-3 rounded-lg resize-none" placeholder="Describe Your key responsibilities and achievements..." />
